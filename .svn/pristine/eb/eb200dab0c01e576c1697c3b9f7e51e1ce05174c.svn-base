                    ...
                    <div class="controlList" :style="" v-for="(item,index) in controlList" :key="index"  @click="clickProduct(index)"
                     :style="listActive == index?'background-color:#fff':''">
                        <template v-if=' index < perPage'>
                            <div class="controlListImg"><img src="./../../../static/img/computer.png"></div>
                            <div class="controlListDetail">
                                <!-- 产品名称 -->
                                <div>{{ $parent.controlObj.tit13 }}：{{item.gdiname}}</div>
                                <!-- 产品型号 -->
                                <div>{{ $parent.controlObj.tit14 }}：{{item.subtypes}}</div>
                                <!-- 产品ID -->
                                <div>{{ $parent.controlObj.tit15 }}：{{item.gditid}}</div>
...
import { getTpopedomList,getFinishedList,getLimittimeList,saveGoodsppd } from "@/api/https"
import child from "./../../components/page.vue";
export default {
    data(){
        return{
            list:[{router: this.$parent.controlObj.tit03}],
            end: this.$parent.controlObj.tit01,
            windowHeight:'',
            controlList:[],
            sonPopedom: [],
            // 复选框
    ...
    methods:{
            ...
            var obj = {
                ...
                success:res => {  //res是server端响应
                    if (res.data.message.indexOf("getSuccess") > -1) {
                        this.controlList = [];
                        var array = res.data.goodsitemList;
                ...
                success:res => {  //res是server端响应
                    if (res.data.message.indexOf("getSuccess") > -1) {
                        ...
                        for (let index = 0; index < this.sonPopedom.length; index++) {
                            ...
                            this.getLimittimeList('goods',this.controlList[this.listActive].gditid,element.pdmid,element)
                        }
                    } else {
                        this.$message.error(this.$promptObj.searchfail);
                    }
        ...
        saveGoodsppd(item,time,flag){
            ...
            obj = {
                types: 'goods',
                tbid: this.controlList[this.listActive].gditid,
                pdmid: item.pdmid,
