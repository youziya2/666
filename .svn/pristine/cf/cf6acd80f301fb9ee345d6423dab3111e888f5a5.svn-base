        ...
        <div class="theme-content">
            <div class="theme-filtrate">
               <!-- 条件筛选 -->
                ...
                <div class="cut">
                    <el-checkbox v-model="allCheck" v-if='themelist.length > 0' @change="allChoice">全选</el-checkbox>
                    <div v-else class="content-null"> 暂无数据 </div>
                </div>
            </div>
            <el-scrollbar :style="{height:windowHeight}">
                <el-row :gutter="20" style="min-height: 520px;">
                    <el-col :span="6" v-for="(item,index) in themelist" :key="index" class="desktopListImg">
                        <div class="font-fffback" :class="item.themecheck?'active':''" @mouseenter="productActive = index" @mouseleave="productActive = ''">
                            <el-checkbox name="checkcolor" v-model="item.themecheck" v-show="productActive === index||item.themecheck"
                             @change="getChoice(totalChoice,index,item.themecheck,item.dpid)">
                            </el-checkbox>
                            <div class="themeLists" :title='item.dpcname'>
                                <img src="./../../../../static/img/themefont.png">
...
import { fontList  } from "@/init/desktop.js";
export default {
    data(){
        return{
            filname:[],
