    ...
    <div>
        ...
        <div class="list-content">
            <div>
                ...
                <div style="overflow: hidden;line-height: 34px;padding-bottom: 45px;">
                    <!-- 产品搜索 -->
                    <span class="search">{{$parent.finishedObj.tit04}}</span>
                        <div class="search-condition">
                            <!-- 产品标准 -->
                            <div class="grid-content bg-purple" style="text-align:right;width:130px">{{$parent.finishedObj.tit10}}</div>
                            <div class="discrible" style="width:50%">
                                <el-select
                                v-model="gdid"
                                :placeholder="gdid ? gdid : $buttonObj.select "
                                >
                                <el-option
                        ...
                        <div class="search-condition" style="width:300px">
                            <el-input :placeholder="$parent.finishedObj.tit09" style="width:100%" v-model="scct" class="input-with-select"></el-input>
                        </div> 
                        <div class="search-condition">
                ...
                <div class="productListbox">
                    <el-row :gutter="20" v-if="type == 1">
                        <el-col :span="6" v-for="(item,index) in product" :key="index" class="productListsImg">
                            <div class="product-fffback" :class="item.listCheck?'productActive':''" @mouseenter="productActive = index" @mouseleave="productActive = ''">
                                <el-checkbox name="checkcolor" v-model="item.listCheck"  @change="getChoice(totalChoice,index,item.listCheck,item.gditid)" 
                                v-show="productActive === index||item.listCheck"></el-checkbox>
                                <div class="productLists" :title='item.gdiname'>
                                    <img src="../../../../static/img/computer.png" @click="goxiugai(item.gdiname,item,'gditid')">
                                    <div class="txt-hidden">{{item.gdiname}}</div>
                                </div>
                            </div>
                        </el-col>
                    </el-row>
                    <ul class="productTypeTwo" v-if="type == 2">
                        <li v-for="(item,index) in product" :key="index" @mouseenter="listActive = index" @mouseleave="listActive = ''" :class="item.listCheck?'listBack':''">
                            <div class="listCheck">
                                <el-checkbox v-model="item.listCheck"  v-show="listActive === index||item.listCheck"  @change="getChoice(totalChoice,index,item.listCheck,item.gditid)"></el-checkbox>
                            </div>
                            <div class="listImg">
                                <img src="../../../../static/img/computer.png">
                            </div>
                            <div class="particulars" @click="goxiugai(item.gdiname,item,'gditid')">
                                <div>{{item.gdiname}}</div>
...
<script>
import { batchDelete,getProductList,getFinishedList,getTemplate } from "../../../api/https.js";
...
import { finishedListInit } from "@/init/product.js";
export default {
    data(){
        return{
            centerDialogVisible: false,
            list:[{router: this.$parent.finishedObj.tit01}],
            end: this.$parent.finishedObj.tit03,
            selectNum:0,//选中的数量
            totalChoice:[],//点击数组
            totalChecked:[],//选择数组
            scct:'',//搜索内容
    ...
    methods:{
        getTemplate(){
            var obj = {
                success:response => {
                    ...
                    var arr = response.data.goodsList ,obj = {};
        ...
        allChoice(){
            this.product.forEach(element => {
                if (this.allCheck) {
                    ...
                    this.totalChoice.push(element.gditid);
                } else {
        ...
        doChoice(arr1,arr2){
            ...
            this.$bus.emit('deleteArr',this[arr2],'gditid')
        },
        getHeight(){
        ...
        goxiugai(name,obj,gdid){
            var add = goDetail(name,obj,'/management/productList/finishedDetail/',"./static/img/smallyy.png",gdid)
        ...
        delOne(){
            ...
            batchDelete('BHGoodsServiceInfc.batchRemoveGoodsitem','goodsitem',obj)
        },
        // 数据请求
        getData(){
            var obj = {
                ...
                success:response => {
                    if (response.data.message.indexOf("getSuccess") > -1) {
                        var rdg =  response.data.goodsitemList , obj = {}
                        this.product = []
