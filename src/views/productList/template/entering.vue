  ...
  <div>
    ...
    <el-scrollbar :style="{height:windowHeight}">
            ...
            <el-col :span="6" v-for="(item,index) in parvalObj" :key="index" style="margin-bottom: 24px;">
              ...
              <div class="discrible" :class="{'must':item.must, 'mycolor': index == 'colors'}">
                <template v-if="index == 'colors'">
                  <el-color-picker :disabled="isShow" :style="'text-align:right;cursor:auto;background-color:'+item.val" class="enter-input" v-model="item.val"></el-color-picker>
                </template>
                ...
                <template v-else-if="item.select">
                  ...
                  :disabled="isShow"
                    v-model="item.val"
                    style="width:100%"
                    :placeholder="$parent.templateObj.tit18"
                  >
                    <el-option
                ...
                <template v-else>
                  <template v-if ="index == 'gdcname'">
                    <input class="enter-input" :disabled="isShow" v-model="item.val" @keyup = 'getGoodsCount'>
                  </template>
                  <template v-else>
                    <input class="enter-input" :disabled="isShow" v-model="item.val" >
                  </template>       
                </template>               
      ...
      <div class="xz-content">
        ...
        <div class="input">
          <el-row :gutter="20">
            <el-col :span="6" v-for="(item,index) in cpuObj" :key="index" style="margin-bottom: 24px;">
              <div class="grid-content bg-purple">{{item.name}}</div>
      ...
      <div class="xz-content">
        ...
        <div class="input">
          <el-row :gutter="20">
           <el-col :span="6" v-for="(item,index) in memeryObj" :key="index" style="margin-bottom: 24px;">
              <div class="grid-content bg-purple">{{item.name}}</div>
              <div class="discrible" :class="{'must':item.must}">
                <template v-if="item.select">
                  <el-select
...
import { enterInit } from "@/init/product.js";
...
export default {
  data() {
    return {
      ...
      end: this.$parent.templateObj.tit02,
      dialogVisible: false,
      parvalObj: {},//产品基本信息
      cpuObj: {},//cpu
      memeryObj: {},//内存
      ...
      windowHeight: "",
      goodsObj: "",
      fatherObj: {id:'01',isShow:true,spaceNum:0,timeNum:-1,removeTime:true}
    };
  },
  created() {
    ...
    this.getHeight();
    enterInit(this);
    this.parvalObj.savetime.val = getNowFormatDate(new Date());
  },
  components:{
    sliderm
  },
  ...
  methods: {
    ...
    // 点击完成
      carryOut(){
        this.$router.push({ path: "/management/productList/lists" });
      },
      // 检验成品名称
    getGoodsCount(){
      var obj = {
    ...
    clearContent() {
      for (const key in this.parvalObj) {
        if( key == "types" ||  key == "colors" || key == "savetime") continue
        this.parvalObj[key].val = "";
      }
      for (const key in this.cpuObj) {
        if( key == "cpubits" ) continue
        this.cpuObj[key].val = "";
      }
      for (const key in this.memeryObj) {
        this.memeryObj[key].val = "";
      }
      this.content = "";
    },
    getHeight() {
