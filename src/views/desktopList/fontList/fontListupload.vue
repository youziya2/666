<template>
    <div class="theme-content">
        <el-scrollbar :style="{height:windowHeight}">
            <!-- 字体信息 -->
            <div class="theme-title">{{$parent.$parent.desktop.tit28}}</div>
            ...
            <div v-if="isShow">
                <!-- 字体文件 -->
                <div class="theme-title">{{$parent.$parent.desktop.tit29}}</div>
...
import { enterFont } from "@/init/desktop.js";
...
export default {
    ...
    methods: {
        // 点击完成
        carryOut() {
            this.$router.push({ path: "/management/desktopList/fontList/fontList" });
        },
        getHeight(){
            ...
            var obj = {
                ...
                success: response => {
                    if (response.data.message.indexOf("getSuccess") > -1) {
                        ...
                        this.isHave = true;
                        this.$message.error( this.$parent.$parent.desktop.tit33 );
                        } else {
                        this.isHave = false;
                        }
                    } else {
        ...
        // 点击发送请求
        addDesktop(){
            if(this.isHave) return this.$message.error( this.$parent.$parent.desktop.tit33 );
