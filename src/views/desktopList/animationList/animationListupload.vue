<template>
    <div class="theme-content">
        <el-scrollbar :style="{height:windowHeight}">
            <!-- 开机视频信息 -->
            <div class="theme-title">{{$parent.$parent.desktop.tit03}}</div>
            ...
            <div v-if="isShow">
                <!-- 视频文件 -->
                <div class="theme-title">{{$parent.$parent.desktop.tit23}}</div>
...
import {  enterAnimation } from "@/init/desktop.js";
...
export default {
    ...
    methods: {
        // 点击完成
        carryOut() {
            this.$router.push({ path: "/management/desktopList/animationList/animationList" });
        },
        getHeight(){
            ...
            var obj = {
                ...
                success: response => {
                    if (response.data.message.indexOf("getSuccess") > -1) {
                        ...
                        this.isHave = true;
                        this.$message.error( this.$parent.$parent.desktop.tit31 );
                        } else {
                        this.isHave = false;
                        }
                    } else {
        ...
        // 点击发送请求
        addDesktop(){
            if(this.isHave) return this.$message.error( this.$parent.$parent.desktop.tit31 );
