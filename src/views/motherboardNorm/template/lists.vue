            ...
            <div>
                ...
                <div class="search-mb">
                    ...
                    <div class='search-title'>
                        <span class="grid-content">{{$parent.mainboardObj.tit03}}</span>
                    </div>
                    <div class='search-option'>
                            <!-- 主板大类 -->
                        <span class="grid-content">{{$parent.mainboardObj.tit10}}</span>
                        <el-select :placeholder="$buttonObj.select" v-model = 'types' style = 'width:60%'  @change="changeType">
                            <el-option
                            v-for="(item,k) in typesObj"
                            :key="k"
                            :label="item"
                            :value="k">
                            </el-option>
                    ...
                    <div class='search-option'>
                            <!-- 主板子类 -->
                        <span class="grid-content">{{$parent.mainboardObj.tit12}}</span>
                        <el-select :placeholder="$buttonObj.select" v-model = 'subtypes' style = 'width:60%'>
                            <el-option
                            v-for="(item,k) in subtypesObj[types]"
                            :key="k"
                            :label="item"
                            :value="k">
                            </el-option>
                    ...
                    <div class='search-option'>
                            <!-- 主板型号 -->
                        <span class="grid-content">{{$parent.mainboardObj.tit11}}</span>
                        <el-input v-model="modnum" :placeholder="$parent.mainboardObj.tit11" style = 'width:60%'></el-input>    
                    </div>
                    <el-button icon="el-icon-search" @click.native="doSearch()" :style="{backgroundColor:colorselects}"></el-button>
                </div>
                <div class="cut">
                        <!-- 全选 -->
                ...
                <div class="productListbox">
                    <el-row :gutter="20" v-if="type == 1">
                        <el-col :span="6" v-for="(item,index) in product" :key="index" class="productListsImg">
                            <div class="product-fffback" :class="item.listCheck?'productActive':''" @mouseenter="productActive = index" @mouseleave="productActive = ''">
                                <el-checkbox name="checkcolor" v-model="item.listCheck"  @change="getChoice(totalChoice,index,item.listCheck,item.mbid)" 
                                v-show="productActive === index||item.listCheck"></el-checkbox>
                                <div class="productLists" :title='item.mbname'>
                                    <img src="../../../../static/img/zhuban.jpg" @click="goxiugai(item.mbname,item,'mbid')">
                                    <div class="txt-hidden">{{item.mbname}}</div>
                                </div>
                            </div>
                        </el-col>
                    </el-row>
                    <ul class="productTypeTwo" v-if="type == 2">
                        <li v-for="(item,index) in product" :key="index" @mouseenter="listActive = index" @mouseleave="listActive = ''" :class="item.listCheck?'listBack':''">
                            <div class="listCheck">
                                <el-checkbox v-model="item.listCheck"  v-show="listActive === index||item.listCheck"  @change="getChoice(totalChoice,index,item.listCheck,item.mbid)"></el-checkbox>
                            </div>
                            <div class="listImg">
                                <img src="../../../../static/img/zhuban.jpg">
                            </div>
                            <div class="particulars" @click="goxiugai(item.mbname,item,'mbid')">
                                <div>{{item.mbname}}</div>
...
import { batchDelete,mainboardList } from "../../../api/https.js";
...
export default {
    data(){
        return{
            ...
            end: this.$parent.mainboardObj.tit02,
            selectNum:0,//选中的数量
            totalChoice:[],//点击数组
            totalChecked:[],//选择数组
            modnum: '',//主板型号
    ...
    methods:{
        ...
        allChoice(){
            this.product.forEach(element => {
                if (this.allCheck) {
                    ...
                    this.totalChoice.push(element.mbid);
                } else {
        ...
        doChoice(arr1,arr2){
            ...
            this.$bus.emit('deleteArr',this[arr2],'mbid')
        },
        getHeight(){
        ...
        goxiugai(name,obj,gdid){
            var add = goDetail(name,obj,'/management/motherboardNorm/productDetails/',"./static/img/smallyy.png",gdid)
        ...
        delOne(){
            ...
            batchDelete('BHMainboardServiceInfc.batchRemoveMainboard','mainboard',obj)
        },
        // 数据请求
        getData(){
            this.product = []
            var obj = {
                ...
                success:response => {
                    if (response.data.message.indexOf("getSuccess") > -1) {
                        var rdg =  response.data.mainboardList , obj = {}
...
<style scoped>
    ...
    .search-mb .el-input-group__append{
        width: 19px;
        left: 20px;
    }
    .search-mb .el-input-group__append button{
        height: 40px;
        padding-top: 9px;
    }
    .search-mb .el-input-group__append i{
        font-size: 20px;;
        color: #fff;
    }
    .search-mb ::-webkit-input-placeholder { /* WebKit browsers */
        color: #999;
        font-size: 16px;
        font-weight: 700;
    }
