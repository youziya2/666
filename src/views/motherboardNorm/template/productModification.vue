              ...
              <div class="discrible" :class="{'must':item.must, 'mycolor': index == 'colors'}">
                ...
                <template v-else>
                  <template v-if ="index == 'mbname' || index == 'modnum'">
                    <input class="enter-input" v-model="item.val" @keyup = "getMainboardCount(index == 'mbname' ? 'isMbname':'isModnum' , index)">
                  </template>
                  <template v-else>
          ...
          <div class="btn-group">
            <el-row>
              ...
              <el-button
                ...
                @click="cancel('/management/motherboardNorm/productDetails/')"
...
import { templateUpdate } from "@/init/mainboard.js";
export default {
  inject: ["replaceroute"],
  data() {
    return {
      ...
      list: [ { router: this.$parent.mainboardObj.tit01 }, { router: this.$parent.mainboardObj.tit08 }],
      end: '',
      dialogVisible: false,
      parvalObj: {},//产品基本信息
      typesObj:{},
  ...
  created() {
    ...
    this.getHeight();
    this.parvalObj.types.select = this.typesObj;  
  ...
  methods: {
    ...
    getInfo() {
      ...
      this.gdid = this.end.substring(this.end.indexOf("@") + 1);
      var obj = JSON.parse(sessionStorage.getItem(this.gdid)), arr = [], str = "img0";
      if(obj){
        arr = obj.imgList || [];
      ...
      for (const key in this.parvalObj) {
        this.parvalObj[key].val = obj[key] ?  obj[key] : ''
      }
      this.parvalObj.subtypes.select = this.subtypesObj[this.parvalObj.types.val];
    },
    // 校验主板名/型号唯一性
    getMainboardCount(boo,str){
      var object = JSON.parse(sessionStorage.getItem(this.gdid));
      if ( this.parvalObj[str].val == object[str]) return this[boo] = false;
      var obj = {
    ...
    sendInfo() {
      ...
      if (checkNull(this.parvalObj)) {
        // if (this.content == "")  return this.$message.error("商品描述不能为空！");
        var obj = {
          self: this,
          success: response => {
            if (response.data.message.indexOf("editSuccess") > -1) {
              this.cancel("/management/motherboardNorm/productDetails/");
            } else {
              this.$message.error(this.$promptObj.savefail);
            }
          }
        };
        updateMainboard(obj, true);
      }
    },
    // 取消修改
    cancel(path) {
